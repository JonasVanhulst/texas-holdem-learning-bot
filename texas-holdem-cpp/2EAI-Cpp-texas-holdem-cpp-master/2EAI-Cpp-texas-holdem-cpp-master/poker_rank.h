...
class PokerRank {
 public:
  ...
  PokerRank(const Card* card) : category(HIGH_CARD), rank(card->getRank()) {
    hand.push_back(card);
  }
  PokerRank(const Card* card1, const Card* card2);
  PokerRank(POKER_HAND category, Card::RANK rank)
  ...
  inline std::string toString(void)
      const;  // inline to speed up the translation (it is a small function)
  ...
  std::vector<const Card*> getHand(void) const {
    return (hand);
  }                           // TO CHECK : const pointer ???
  int handValue(void) const;  // 4 bits for a card, 5 bytes is enough !

  bool operator>(const PokerRank& other) const {
    return (category > other.category ||
            (category == other.category &&
             (rank > other.rank ||
              (rank == other.rank && handValue() > other.handValue()))));
  }
  bool operator<(const PokerRank& other) const {
    return (category < other.category ||
            (category == other.category &&
