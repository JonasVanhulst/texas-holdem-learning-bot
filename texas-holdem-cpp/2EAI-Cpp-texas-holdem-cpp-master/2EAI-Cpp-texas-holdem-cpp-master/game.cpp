...
bool compareShowedCards(const ShowedCard& lhs, const ShowedCard& rhs) {
  // std::cout << "Comp " << lhs.rank.toString() << " vs. " <<
  // rhs.rank.toString() << " = " << (lhs.rank < rhs.rank) << std::endl;
  return (lhs.rank < rhs.rank || (lhs.rank == rhs.rank &&
                                  lhs.player->getBet() > rhs.player->getBet()));
}

void Game::play(size_t winnerAmount) {
  while (players.size() > winnerAmount) {
    plays++;
    std::cout << "Play " << plays << " (" << blind << ")" << std::endl
              << "--------" << std::endl
              << std::endl;
...
void Game::dealPocket(void) {
  size_t i = (dealer + 1) %
             players.size();  // first card is give to player left from dealer
...
void Game::betting(bool isPreFlop) {
  size_t i = (dealer + 1) % players.size();  // first bet starts left from
                                             // dealer
  if (isPreFlop) {
    highestBet = players[i]->raiseBet(
        blind);  // first small blind starts left from dealer
    i = (i + 1) %
        players.size();  // next player on the left in round robin for big blind
    highestBet = std::max(highestBet, players[i]->raiseBet(blind * 2));
    i = (i + 1) % players.size();
  }
  char minimumPlayerCheck = players.size();
  int raisedBet;
  while ((players[i]->getBet() < highestBet) ||
         (amountHands > 1 && minimumPlayerCheck > 0)) {
    ...
    if (players[i]->hasHand() && !players[i]->isAllIn()) {
      ...
      if (raisedBet < 0) {
        ...
        highestBet =
            std::max(highestBet, players[i]->raiseBet(highestBet + raisedBet));
      }
    }
    i = (i + 1) % players.size();  // next player on the left in round robin
  }
...
void Game::showdown(void) {
  ...
  for (size_t i = 0; i < players.size(); i++) {
    if (players[i]->hasHand()) {
      showedCards.push_back(ShowedCard{
          players[i], players[i]->getRank(),
          static_cast<unsigned char>((players.size() + i - 1 - dealer) %
                                     players.size())});
    }
  }
  ...
  if (showedCards.size() == 0) {  // should NOT happen, but OK house wins :D
    std::cerr << "HOUSE WINS !!!" << std::endl;
    // system( "pause" );
  } else if (showedCards.size() == 1) {  // Bluff !
    unsigned int pot = 0;
    for (size_t i = 0; i < players.size(); i++) {
    ...
    while (highestBet > 0 && showedCards.size() > 0) {
      ...
      for (i = 2; i <= showedCards.size() &&
                  (showedCards[showedCards.size() - i].rank ==
                   showedCards.back().rank);
           i++);  // i will be the amount of showedCards that will need to split
                  // the pot
      i--;  // we found the one that was not comform
      if (i > 1) {
#ifdef FULL_PRINT
        std::cout << i << " players need to split the pot of " << pot
                  << std::endl;
#endif
        ...
        if (rest > 0) {
          ...
          for (i = 1; i <= showedCards.size() &&
                      (showedCards[showedCards.size() - i].rank ==
                       showedCards.back().rank);
               i++) {
            if (showedCards[showedCards.size() - i].placeAfterDealer <
                placeAfterDealer) {
              luckyBastard = showedCards[showedCards.size() - i].player;
              placeAfterDealer =
                  showedCards[showedCards.size() - i].placeAfterDealer;
            }
          }
          if (luckyBastard != nullptr) {
            ...
            std::cout << " is a lucky bastard and gets the rest (" << rest
                      << ") of the split pot" << std::endl;
#endif
      ...
      while (showedCards.size() > 0 &&
             showedCards.back().player->getBet() == 0) {
