...
class Game {
 ...
 public:
  Game(int blind = 1)
      : dealer(0), blind(blind), highestBet(0), amountHands(0), plays(0) {}
  ...
  size_t getPlayerLocation(const Player* player) const {
    for (size_t i = 0; i < players.size(); i++)
      if (players.at(i) == player) return (i);
    return (-1);
  }
  size_t getDistanceToDealer(const Player* player) const {
    return ((players.size() + (getPlayerLocation(player) - dealer)) %
            players.size());
  }
