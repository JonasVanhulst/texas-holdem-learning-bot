...
int Bart::willYouRaise(unsigned int totalBet) {
  ...
  switch (instance) {
    case 0:
    case 1:
      if (getGame()->getDistanceToDealer(this) < 3) return betSafely(0);
      if (!stopRaising && bluff()) return betSafely(20);
      return betSafely(getGame()->getBlind() * 2);
      break;
    case 2:
    case 3:
      if (getGame()->getDealer()->getID() == PXL_2021 &&
          getGame()->getBlind() > 2)
        return (-1);
      return (0);
      break;
...
bool Bart::bluff() { return ((rand() % 10) == 0); }

int Bart::betSafely(int amount) {
  if (amount < getGame()->getHighestBet()) {
    if (getGame()->getHighestBet() - amount >
        ((2 + instance) * getGame()->getBlind())) {
      return -1;
    } else {
      stopRaising = true;
      return 0;
