...
Tournament::~Tournament() {
  ...
  for (std::vector<std::vector<Game*>>::iterator it = gameRounds.begin();
       it != gameRounds.end(); ++it) {
    for (std::vector<Game*>::iterator itsit = it->begin(); itsit != it->end();
         ++itsit) {
      delete (*itsit);
    }
    it->clear();
...
const Player* Tournament::start(void) {
  ...
  while (gameRounds[round].size() > 1) {
    ...
    for (unsigned char game = 0; game < gameRounds[round].size(); game++) {
      ...
      highestBlind =
          std::max(highestBlind, gameRounds[round][game]->getBlind());
    }
    //		system( "pause" );
    ...
    */
    ...
    for (unsigned char game = 0; game < gameRounds[round].size(); game++) {
      if (game != 0 &&
          ((game % 4 == 0 && (gameRounds[round].size() - game) != 1) ||
           (game % 4 == 3 && (gameRounds[round].size() - game) == 3))) {
        gameRounds[round + 1].push_back(newGame);
        newGame = new Game(highestBlind);
      }
      for (unsigned char i = 0;
           i < gameRounds[round][game]->getPlayers().size(); i++) {
  ...
  std::cout
      << std::endl
      << "With " << gameRounds[round].front()->getPlayers().front()->getChips()
      << " chips !" << std::endl
      << "ID = "
      << (unsigned int)gameRounds[round].front()->getPlayers().front()->getID()
      << std::endl
      << std::endl;
  //	system( "pause" );
...
inline void Tournament::initiatePools(void) {
  gameRounds.push_back(std::vector<Game*>());
  while (players.size() > 0) {  // make groups of 8 players
    ...
    if (players.size() < 20)  // the last group will be smaller than 6 !
    {
      ...
      */
      ...
      switch (players.size()) {
        ...
        case 11:
          std::cerr
              << "TOURNAMENT IS UNFAIR, 11 players should not have happend, "
                 "added players while tournament already started ?"
              << std::endl;
          // fall through
        default:
    ...
    for (unsigned char i = 0; i < groupSize;
         i++)  // everything between 10-6 is ok
    {
