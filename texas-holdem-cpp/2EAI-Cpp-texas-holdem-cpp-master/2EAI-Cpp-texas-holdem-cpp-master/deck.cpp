...
void Deck::printAll(void) {
  for (std::list<const Card*>::iterator it = deck.begin(); it != deck.end();
       ++it)  // NOTE : iterator is redeclaired and will not effect the member
              // variable
  {
    if (it == iterator) {
      std::cout << "[" << *it << "]";
...
inline void Deck::cut(char amount)  // can only be place in cpp IF it is only
                                    // used there ! (must be private than)
{
  if (amount == 0) {
    amount = (1 + rand()) % 52;
  }
