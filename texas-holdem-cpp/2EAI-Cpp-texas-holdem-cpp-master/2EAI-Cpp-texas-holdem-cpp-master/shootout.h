...
void runStatistics(Tournament& tournament, int amountOfTournaments) {
  ...
  for (int i = 0; i < amountOfTournaments; i++) {
    ...
    std::cerr << winnaar->getName() << "(" << (short)winnaar->getInstance()
              << ")";
    std::cerr << std::endl
              << "With " << winnaar->getChips() << " chips !" << std::endl
              << "ID = " << (unsigned int)winnaar->getID() << std::endl
              << std::endl;
  }

  for (int id = CUSTOM; id < MAX_PLAYER_ID; id++) {
    int sum = 0;
    for (std::map<const Player*, int>::iterator it = wins.begin();
         it != wins.end(); ++it) {
      const Player* player = it->first;
      if (player->getID() == id) {
        std::cerr << "ID: " << player->getID()
                  << ", Name: " << player->getName()
                  << ", Instance: " << (short)player->getInstance()
                  << ", Wins: " << it->second << std::endl;
        sum += it->second;
      }
    }
    if (sum > 0) {
  ...
  //		for( int j = 0 ; j < 4 ; j++ )
  //		{
  //			std::cerr << "ID: " << ( i + PXL_2019 ) << ", Instance:
  //" << j << ", Wins: " << wins[i][j] << std::endl; 			sum += wins[i][j];
  //		}
  //		std::cerr << "ID: " << ( i + PXL_2019 ) << ", Total Wins: " <<
  //sum << std::endl;
  //	}
}
