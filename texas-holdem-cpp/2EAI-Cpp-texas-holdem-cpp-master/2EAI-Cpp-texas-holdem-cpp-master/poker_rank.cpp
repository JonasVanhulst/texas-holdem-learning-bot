...
PokerRank::PokerRank(std::vector<const Card*> combo) {
  ...
  // STRAIGHT_FLUSH ?
  {
         ...
         s++)  // find first flush as 5 or more of the same cards...
    {
      if (suitCount[s - 0x3] >= 5) {  // FLUSH !
        ...
        for (byte i = 0; i < (byte)combo.size() && combo.at(i)->getRank() == 1;
                   ...
                   // subsequence, but from high to lower
        {
          if (combo.at(i)->getSuit() == s)  // Only Flush Straight Check
          {
                ...
                nextRankShouldBe)  // It is the next card we expected ?
            {
              ...
              rank = combo.at(i)->getRank();  // We start from the highest rank,
                                              // that one we need to remember !
            }
            nextRankShouldBe =
                combo.at(i)->getRank() - 1;  // We go from high to low
          }
        }
      }
    }
  ...
  Card::RANK valueOfTrio = 0;
  {
         ...
         i++)  // Check all Aces first
    {
                     ...
                     // highest, so pack it an ship it !
      {
        ...
        if (combo.at(combo.size() - 1)->getRank() !=
    ...
    for (byte i = combo.size() - 2; i >= 0 && combo.at(i)->getRank() != 1;
               ...
               // to low
    {
      if (rank == combo.at(i)->getRank()) {
        ...
        {
          valueOfPair =
              combo.at(i)->getRank();  // Current rank is part of a pair
        } else if (valueOfSecondPair == 0 &&
                   valueOfPair !=
                       combo.at(i)->getRank())  // No second pair was found
                                                // before, but this one is one !
        {
          valueOfSecondPair =
              combo.at(i)->getRank();  // Current rank is part of a pair
        }
        ...
        {
          valueOfTrio =
              combo.at(i)->getRank();  // Current rank is part of a trio
          if (valueOfPair ==
              combo.at(i)->getRank())  // clean out the pair that bacame a trio
          {
            valueOfPair = 0;
        ...
        {
          // Add highest card as kicker !
          if (combo.at(0)->getRank() ==
              1)  // There is an Ace ! And we are not taking Aces in this part
          {
            hand.push_back(combo.at(0));
          } else if (combo.at(combo.size() - 1)->getRank() !=
                     combo.at(i)->getRank())  // if the highest card is not one
                                              // of the four, add this
          {
            hand.push_back(combo.at(combo.size() - 1));
        ...
        rank = combo.at(i)->getRank();  // We start from the highest rank, that
  ...
  // FLUSH ?
  {
         ...
         s++)  // find first flush as 5 or more of the same cards...
    {
      if (suitCount[s - 0x3] >= 5) {  // FLUSH !
        ...
        for (byte i = combo.size() - 1; i >= 0 && combo.at(i)->getRank() != 1;
  ...
  // STRAIGHT?
  {
    ...
    // Ace can be high AND low card !
    if (combo.at(0)->getRank() ==
        1)  // If we have an Ace, it must be in the front
    {
      hand.push_back(combo.at(0));
      nextRankShouldBe = 13;  // We need a KING !
      rank = 14;              // Ace !
               ...
               // lower
    {
          ...
          nextRankShouldBe)  // It is the next card we expected ?
      {
        ...
        sequencingNumbers++;
        nextRankShouldBe = combo.at(i)->getRank() - 1;  // We go from high to
                                                        // low
      } else if (combo.at(i)->getRank() - 1 !=
  ...
  // TWO_PAIR ?
  {
    if (valueOfSecondPair > 0) {
      ...
      {
        if (combo.at(i)->getRank() != valueOfPair &&
            combo.at(i)->getRank() != valueOfSecondPair) {
          hand.push_back(combo.at(i));  // Add the highest rest
          return;                       // we have 5 pack it, ship it !
        }
      }
