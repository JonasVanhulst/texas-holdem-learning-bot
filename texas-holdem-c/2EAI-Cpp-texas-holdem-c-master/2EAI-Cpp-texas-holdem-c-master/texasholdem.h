...
enum COMPARE { LOWER = -1, EQUAL = 0, HIGHER = 1 };

enum SUIT { SPADES = 0x6, DIAMONDS = 0x4, CLUBS = 0x5, HEARTS = 0x3 };
...
int willYouRaise(struct Game* game, struct Player* player,
                 unsigned int totalBet);  // TO BE DEFINED !!!
...
enum COMPARE comparePokerRanks(struct PokerRank lhs, struct PokerRank rhs) {
  if (lhs.category < rhs.category ||
      (lhs.category == rhs.category &&
       ((lhs.rank < rhs.rank && lhs.rank != 1) ||
        (lhs.rank == rhs.rank && handValue(lhs) < handValue(rhs))))) {
    return (LOWER);
  } else if (lhs.category > rhs.category ||
             (lhs.category == rhs.category &&
              ((lhs.rank > rhs.rank && rhs.rank != 1) ||
               (lhs.rank == rhs.rank && handValue(lhs) > handValue(rhs))))) {
    return (HIGHER);
  } else {
    return (EQUAL);
...
PokerRank getMyHandRank(struct Hand* hand) {
  ...
  // STRAIGHT_FLUSH ?
  {
    for (byte s = 0x3; s <= 0x6;
         s++)  // find first flush as 5 or more of the same cards...
    {
      if (suitCount[s - 0x3] >= 5) {  // FLUSH !
        // STRAIGHT_FLUSH ?
        byte sequencingNumbers = 1;
        RANK nextRankShouldBe = 0xFF;  // for overflow !
        // Ace can be high AND low card !
        for (byte i = 0; i < comboSize && combo[i]->rank == 1;
             i++)  // Check all Aces for the correct Suit
        {
        ...
        for (byte i = comboSize - 1; i >= 0;
             i--)  // check all card that have the same suit if they are in a
                   // subsequence, but from high to lower
        {
          if (combo[i]->suit == s)  // Only Flush Straight Check
          {
            if (combo[i]->rank ==
                nextRankShouldBe)  // It is the next card we expected ?
            {
              returnValue.hand[handIndex] = combo[i];
              handIndex++;
              if (sequencingNumbers ==
                  4)  // We found the fifth subsequencing card, starting from
                      // the highest, so pack it an ship it !
              {
              ...
              handIndex++;
              sequencingNumbers =
                  1;  // Start all over as it is not the next card in line
              returnValue.rank =
                  combo[i]->rank;  // We start from the highest rank, that one
                                   // we need to remember !
            }
  ...
  RANK valueOfTrio = 0;
  {
    ...
    handIndex = 0;
    for (byte i = 0; i < comboSize && combo[i]->rank == 1;
         i++)  // Check all Aces first
    {
      returnValue.hand[handIndex] = combo[i];
      handIndex++;
      if (identicalNumbers == 1) {
        valueOfPair = 14;  // Pair of Aces
      } else if (identicalNumbers ==
                 2)  // No trio was found before, but this one is one !
      {
        valueOfTrio = 14;  // Trio of Aces
        valueOfPair = 0;   // not a pair...
      } else if (identicalNumbers ==
                 3)  // We found the fourth identical rank, starting from the
                     // highest, so pack it an ship it !
      {
        returnValue.category = FOUR_OF_A_KIND;
        returnValue.rank = 14;  // Four Aces
        // Add highest card as kicker !
        if (combo[comboSize - 1]->rank !=
            1)  // cannot be another Ace ! and if the last is Ace, there is no
                // higher card.
        {
    ...
    for (byte i = comboSize - 2; i >= 0 && combo[i]->rank != 1;
         i--)  // chech all the oher cards for four of the same rank, from high
               // to low
    {
      if (returnValue.rank == combo[i]->rank) {
        ...
        handIndex++;
        if (valueOfPair ==
            0)  // No pair was found before, but this one is one !
        {
          valueOfPair = combo[i]->rank;  // Current rank is part of a pair
        } else if (valueOfSecondPair == 0 &&
                   valueOfPair !=
                       combo[i]->rank)  // No second pair was found before, but
                                        // this one is one !
        {
          valueOfSecondPair = combo[i]->rank;  // Current rank is part of a pair
        }
        if (identicalNumbers == 2 &&
            valueOfTrio ==
                0)  // No trio was found before, but this one is one !
        {
          valueOfTrio = combo[i]->rank;  // Current rank is part of a trio
          if (valueOfPair ==
              combo[i]->rank)  // clean out the pair that bacame a trio
          {
            valueOfPair = 0;
                ...
                // highest, so pack it an ship it !
        {
          // Add highest card as kicker !
          if (combo[0]->rank ==
              1)  // There is an Ace ! And we are not taking Aces in this part
          {
            returnValue.hand[handIndex] = combo[0];
            handIndex++;
          } else if (combo[comboSize - 1]->rank !=
                     combo[i]->rank)  // if the highest card is not one of the
                                      // four, add this
          {
            returnValue.hand[handIndex] = combo[comboSize - 1];
            handIndex++;
        ...
        returnValue.rank = combo[i]->rank;  // We start from the highest rank,
                                            // that one we need to remember !
      }
    }
  }
  ...
  // FULL_HOUSE ?
  {
    if (valueOfTrio > 0 && valueOfPair > 0) {
      ...
      {
        for (byte i = 0; i < comboSize && combo[i]->rank == 1; i++) {
          returnValue.hand[handIndex] =
              combo[i];  // Add the first Pair of Aces !
          handIndex++;
          if (handIndex == 5) {
  ...
  // FLUSH ?
  {
         ...
         s++)  // find first flush as 5 or more of the same cards...
    {
      if (suitCount[s - 0x3] >= 5) {  // FLUSH !
        ...
        for (byte i = comboSize - 1; i >= 0 && combo[i]->rank != 1;
             i--)  // In reverse order we will find the highest rank with the
                   // correct suit
        {
          if (combo[i]->suit == s)  // Only Flush Check
          {
            ...
            if (handIndex ==
                5)  // Stop when we have the 5 highest cards of the same suit !
            {
              return (returnValue);
            }
          }
  ...
  // STRAIGHT?
  {
    ...
    for (byte i = comboSize - 1; i >= 0;
         i--)  // check all card if they are in a subsequence, but from high to
               // lower
    {
      if (combo[i]->rank ==
          nextRankShouldBe)  // It is the next card we expected ?
      {
        ...
        handIndex++;
        if (sequencingNumbers ==
            4)  // We found the fifth subsequencing card, starting from the
                // highest, so pack it an ship it !
        {
        ...
        sequencingNumbers++;
        nextRankShouldBe = combo[i]->rank - 1;  // We go from high to low
      } else if (combo[i]->rank - 1 !=
                 nextRankShouldBe)  // The rank should not be the same as
                                    // previous, else skip this card
      {
        handIndex = 0;
        returnValue.hand[handIndex] = combo[i];
        handIndex++;
  ...
  // TWO_PAIR ?
  {
    if (valueOfSecondPair > 0) {
      ...
      {
        if (combo[i]->rank != valueOfPair &&
            combo[i]->rank != valueOfSecondPair) {
          returnValue.hand[handIndex] = combo[i];  // Add the highest rest
          handIndex++;
          return (returnValue);  // we have 5 pack it, ship it !
        }
  ...
  // HIGH_CARD !
  {
    ...
    if (combo[0]->rank == 1)  // If we have an Ace, it must be in the front
    {
      ...
      returnValue.rank = combo[comboSize - 1]->rank;  // Last card is the
                                                      // highest
    }
    for (byte i = comboSize - 1; i >= 0 && combo[i]->rank != 1; i--) {
      returnValue.hand[handIndex] = combo[i];  // Add the highest rest
...
void playerFold(struct Player* player) { clearHand(player->hand); }
...
unsigned int playerRaiseBet(struct Player* player, unsigned int totalBet) {
  ...
  return (
      player
          ->bet);  // Check if raise is possible ? Just take the minimum value !
}
...
void dealPocketInGame(struct Game* game) {
  byte i = (game->dealer + 1) %
           game->playersSize;  // first card is give to player left from dealer
...
void bettingInGame(struct Game* game, byte isPreFlop) {
  size_t i = (game->dealer + 1) %
             game->playersSize;  // first bet starts left from dealer
  if (isPreFlop > 0) {
    game->highestBet = playerRaiseBet(
        game->players[i],
        game->blind);  // first small blind starts left from dealer
    i = (i + 1) % game->playersSize;  // next player on the left in round robin
                                      // for big blind
    game->highestBet = std::max(
        game->highestBet, playerRaiseBet(game->players[i], game->blind * 2));
    i = (i + 1) % game->playersSize;
  }
  char minimumPlayerCheck = game->playersSize;
  int raisedBet;
  while ((game->players[i]->bet < game->highestBet) ||
         (game->amountHands > 1 && minimumPlayerCheck > 0)) {
    if (minimumPlayerCheck > 0) {
      minimumPlayerCheck--;
    }
    if (isHandFilled(game->players[i]->hand) > 0 &&
        game->players[i]->chips !=
            game->players[i]->bet)  // playing and not all in yet
    {
      ...
      if (raisedBet < 0) {
        ...
        game->highestBet = std::max(
            game->highestBet,
            playerRaiseBet(game->players[i], game->highestBet + raisedBet));
      }
    }
    i = (i + 1) % game->playersSize;  // next player on the left in round robin
  }
...
void showdownInGame(struct Game* game) {
  // Pay up
  struct ShowedCard*
      showedCards[MAX_PLAYERS_IN_GAME];  // = new
                                         // showedCard[MAX_PLAYERS_IN_GAME];
  ...
  for (byte i = 0; i < game->playersSize; i++) {
    if (isHandFilled(game->players[i]->hand) > 0) {
      ...
      showedCards[showedCardsIndex]->rank =
          getMyHandRank(game->players[i]->hand);
      showedCards[showedCardsIndex]->placeAfterDealer =
          (game->playersSize + i - 1 - game->dealer) % game->playersSize;
      showedCardsIndex++;
    }
  }
  ...
  if (showedCardsIndex == 0) {  // should NOT happen, but OK house wins :D
    std::cerr << "HOUSE WINS !!!" << std::endl;
    // system( "pause" );
  } else if (showedCardsIndex == 1) {  // Bluff !
    unsigned int pot = 0;
    for (byte i = 0; i < game->playersSize; i++) {
    ...
    for (byte i = 0; i < showedCardsIndex; i++)  // sort
    {
      for (byte j = i + 1; j < showedCardsIndex; j++) {
        enum COMPARE comp =
            comparePokerRanks(showedCards[i]->rank, showedCards[j]->rank);
        if (comp == HIGHER ||
            (comp == EQUAL &&
             showedCards[i]->player->bet < showedCards[j]->player->bet)) {
    ...
    while (game->highestBet > 0 && showedCardsIndex > 0) {
      ...
      printf("Take bet %d of %s\n",
             showedCards[showedCardsIndex - 1]->player->bet,
             showedCards[showedCardsIndex - 1]->player->name);
#endif
      game->highestBet -= bet;
      byte i;
      ...
      for (i = 2; i <= showedCardsIndex &&
                  (comparePokerRanks(showedCards[showedCardsIndex - i]->rank,
                                     showedCards[showedCardsIndex - 1]->rank) ==
                   EQUAL);
           i++);  // i will be the amount of showedCards that will need to split
                  // the pot
      ...
      if (i > 1) {
...
#ifdef FULL_PRINT
          printf("%s wins %d with ",
                 showedCards[showedCardsIndex - i]->player->name, pot);
          printPokerRank(showedCards[showedCardsIndex - i]->rank);
#endif
          i--;
        }
        if (rest > 0) {
          ...
          for (i = 1; i <= showedCardsIndex &&
                      (comparePokerRanks(
                           showedCards[showedCardsIndex - i]->rank,
                           showedCards[showedCardsIndex - 1]->rank) == EQUAL);
               i++) {
            if (showedCards[showedCardsIndex - i]->placeAfterDealer <
                placeAfterDealer) {
              luckyBastard = showedCards[showedCardsIndex - i]->player;
              placeAfterDealer =
                  showedCards[showedCardsIndex - i]->placeAfterDealer;
            }
          }
...
#ifdef FULL_PRINT
            printf(
                "%s is a lucky bastard and gets the rest (%d) of the split "
                "pot\n",
...
#ifdef FULL_PRINT
        printf("%s has highest hand and wins %d with ",
               showedCards[showedCardsIndex - 1]->player->name, pot);
        printPokerRank(showedCards[showedCardsIndex - 1]->rank);
#endif
      }
      while (showedCardsIndex > 0 &&
             showedCards[showedCardsIndex - 1]->player->bet == 0) {
...
void printGame(struct Game* game) {
  ...
  for (byte i = 0; i < game->playersSize; i++) {
    ...
    printf("(%d) Player %s\n[%d] --> %d\n", i + 1, game->players[i]->name,
           game->players[i]->chips, game->players[i]->bet);
